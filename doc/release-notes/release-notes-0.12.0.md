Litecoin Finance Core version 0.12.0 is now available from:

  <https://bitcoin.org/bin/litecoinfinance-core-0.12.0/>
...
backwards-compatible with pre-0.10 versions of Litecoin Finance Core or other software:
...
pre-0.12 versions of Litecoin Finance Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Litecoin Finance Core version 0.11 or
earlier.
...
ECDSA signatures inside Litecoin Finance transactions now use validation using
...
(https://github.com/litecoinfinance/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Litecoin Finance Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Litecoin Finance Core was to raise the default minimum
relay fee.

Litecoin Finance Core 0.12 will have a strict maximum size on the mempool. The
...
Litecoin Finance Core 0.12 also introduces new default policy limits on the length and
...
Litecoin Finance Core 0.12 nodes. Litecoin Finance Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/litecoinfinance/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Litecoin Finance Core 0.12 does not yet have support for
...
This allows running litecoinfinanced without having to do any manual configuration.
...
Litecoin Finance Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Litecoin Finance Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Litecoin Finance Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Litecoin Finance Core 0.13, and it is at this time undecided whether the
...
Litecoin Finance Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Litecoin Finance Core automatically creates a hidden service to listen on, without
manual configuration. Litecoin Finance Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Litecoin Finance Core is listening, and
...
(default: `0.0002` LTFN/kB).

At all times, Litecoin Finance Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LTFN.
Furthermore, Litecoin Finance Core will never create transactions paying less than
...
`litecoinfinance.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/mraksoll4/litecoinfinance/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/litecoinfinance/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/litecoinfinance/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /litecoinfinancerpc>
        ...
        # optional bypass litecoinfinanced rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< litecoinfinancerpc:<password>
    </Location>
...
Restarting litecoinfinanced will no longer clear out the list of banned peers; instead
...
- #6269 `d4565b6` gitian: Use the new litecoinfinance-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include litecoinfinance-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix litecoinfinance-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of litecoinfinance-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override LITECOINFINANCED and LITECOINFINANCECLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [litecoinfinance-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add LITECOINFINANCED_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library liblitecoinfinanceconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "LTFN" (Ross Nicoll)
