Litecoin Finance Core version 0.11.0 is now available from:

  <https://bitcoin.org/bin/litecoinfinance-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer LTFN/kB fee to be
...
For example, add the following to `litecoinfinance.conf`:
...
Block pruning allows Litecoin Finance Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Litecoin Finance Core will 
...
For an example script see [litecoinfinance-submittx](https://github.com/laanwj/litecoinfinance-submittx).
...
- #5334 `cf87536` liblitecoinfinanceconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new litecoinfinance-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only LitecoinFinanceAmountField
...
- #5809 `46bfbe7` Add litecoinfinance-cli man page
...
- #5849 `41259ca` contrib/init/litecoinfinanced.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename LitecoinFinance-Qt.app to LitecoinFinance-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for litecoinfinance-tx)
