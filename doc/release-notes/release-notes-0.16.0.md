Litecoin Finance Core version 0.16.0 is now available from:

  <https://bitcoincore.org/bin/litecoinfinance-core-0.16.0/>
...
Litecoin Finance Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
litecoinfinance data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Litecoin Finance Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µLTFN" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- litecoinfinance-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- litecoinfinance-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 18332.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest litecoinfinance.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.litecoinfinance.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for litecoinfinance-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to litecoinfinance-cli to allow RPC password to be read from standard input (jharvell)
...
- #12035 `eeb6d52` Change µLTFN to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_litecoinfinance_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` litecoinfinance_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.litecoinfinance doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for litecoinfinance-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in litecoinfinance-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop litecoinfinanced (OmeGak)
...
- #10529 `331352f` Improve litecoinfinanced systemd service file (Flowdalic)
