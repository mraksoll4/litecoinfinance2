...
- [Development guidelines](#development-guidelines)
    - [General Litecoin Finance Core](#general-litecoinfinance-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Litecoin Finance Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play litecoin finances" on the test network, if you
...
Litecoin Finance Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/mraksoll4/litecoinfinance/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_litecoinfinance
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_litecoinfinance --log_level=test_suite
$ valgrind -v --leak-check=full src/litecoinfinanced -printtoconsole
```
...
# A coverage report will now be accessible at `./test_litecoinfinance.coverage/index.html`.
```
...
To profile a running litecoinfinanced process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep litecoinfinanced` -- sleep 60
```
...
Litecoin Finance Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/mraksoll4/litecoinfinance/issues/12691)
...
However, in open source software such as Litecoin Finance Core, where everyone uses
...
pay attention to for reviewers of Litecoin Finance Core code.

General Litecoin Finance Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Litecoin Finance Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `LITECOINFINANCE_FOO_BAR_H`, e.g.

```c++
#ifndef LITECOINFINANCE_FOO_BAR_H
#define LITECOINFINANCE_FOO_BAR_H
...
#endif // LITECOINFINANCE_FOO_BAR_H
```
...
Some of these are maintained by active developers of Litecoin Finance Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Litecoin Finance Core so that they can be integrated
...
file descriptors it will cause problems with LitecoinFinance's `select()` loop, because
...
compatibility between nodes. This happened in Litecoin Finance 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to LitecoinFinance's
...
Commit [`bb81e173`](https://github.com/mraksoll4/litecoinfinance/commit/bb81e173) is an example of a scripted-diff.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `litecoinfinance-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `litecoinfinance-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Litecoin Finance Core, the wallet was always
