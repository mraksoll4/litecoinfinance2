 ...
 */
static const char* LITECOINFINANCE_PID_FILENAME = "litecoinfinanced.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", LITECOINFINANCE_PID_FILENAME)));
}
 ...
 * between litecoinfinanced, and litecoinfinance-qt and non-server tools.
*/
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("litecoinfinance-shutoff");
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", LITECOINFINANCE_CONF_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", LITECOINFINANCE_PID_FILENAME), false, OptionsCategory::OPTIONS);
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("litecoinfinance-loadblk");
...
/** Sanity checks
 *  Ensure that Litecoin Finance is running in a usable environment with all
...
{
    // Make sure only a single Litecoin Finance process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", LITECOINFINANCE_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if litecoin finance is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if litecoin finance is started while in a temporary directory.\n",
