This directory contains the LitecoinFinanceQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
### litecoinfinancegui.(h/cpp)

Represents the main window of the Litecoin Finance UI.
...
ClientModel is used by the main application `litecoinfinancegui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/mraksoll4/litecoinfinance/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a litecoinfinance:... link from a browser).
...
* don't change `local/litecoinfinance_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "litecoinfinance-qt" under "Run", which is where you can also add command line arguments)
